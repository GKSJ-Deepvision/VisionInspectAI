...
export default function SupervisorOverview() {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      try {
        ...
        const [productionRes, trendRes, riskRes, historyRes] = await Promise.all([
        ...
        const critical = historyData.filter(
          (item) => item.severity_level === 'Critical' || item.severity_level === 'High'
        );
        ...
        console.error('Failed to load supervisor overview data:', err);
      } finally {
        setIsLoading(false);
      }
    }
  ...
  const maxDefect = defects.length > 0 ? Math.max(...defects.map((d) => d.count)) : 1;
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="bg-panel border border-gridline p-4">
          ...
          {isLoading ? (
            ...
            <p className="font-display text-2xl text-ink mt-1">
              {Object.keys(risk.category_risk_levels || {}).length}
            </p>
          )}
        </div>
      </div>
              ...
              <div key={name} className="bg-graphite border border-gridline p-4">
                ...
                <div className="w-full h-2 bg-gridline">
                  <div
                    ...
                    style={{ width: `${100 - line.defect_rate_pct}%` }}
                  />
                </div>
              ...
              : trend.map((val, i) => (
                  <div key={i} className="flex-1 flex flex-col items-center gap-2">
                    <div
                      ...
                      style={{ height: `${Math.max(val.score * 100, 5)}%` }}
                      title={`${val.score}%`}
                    />
                    <span className="text-[10px] font-mono text-muted">D{i + 1}</span>
                  </div>
