...
function Bracket({ corner, start, fading }) {
  ...
  const borderStyles = {
    topLeft: { borderTop: '3px solid #60a5fa', borderLeft: '3px solid #60a5fa', borderTopLeftRadius: '6px' },
    topRight: { borderTop: '3px solid #60a5fa', borderRight: '3px solid #60a5fa', borderTopRightRadius: '6px' },
    bottomLeft: { borderBottom: '3px solid #60a5fa', borderLeft: '3px solid #60a5fa', borderBottomLeftRadius: '6px' },
    bottomRight: { borderBottom: '3px solid #60a5fa', borderRight: '3px solid #60a5fa', borderBottomRightRadius: '6px' },
  }[corner]
...
export default function Login() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      setError('')
      await loginUser({ username: name.trim(), password: password.trim(), role })
      navigate('/dashboard')
    } catch (err) {
...
>
      ...
      {startPositions && (
        <>
          <Bracket corner="topLeft" start={startPositions.topLeft} fading={splashFading} />
          <Bracket corner="topRight" start={startPositions.topRight} fading={splashFading} />
          <Bracket corner="bottomLeft" start={startPositions.bottomLeft} fading={splashFading} />
          <Bracket corner="bottomRight" start={startPositions.bottomRight} fading={splashFading} />
        </>
      )}

      {/* Title screen / form — always rendered underneath */}
      {!showForm ? (
        <div className="min-h-screen flex items-center justify-center px-4 relative">
          <div className="text-center max-w-lg">
            <p className="text-gray-400 text-sm tracking-[0.3em] uppercase mb-3">This is</p>
        ...
        <div className="min-h-screen flex items-center justify-center px-4 relative">
          <div className="w-full max-w-md">
            ...
            <h1 className="text-2xl font-bold text-white text-center mb-2">VisionInspect AI</h1>
            <p className="text-gray-400 text-sm text-center mb-6">Manufacturing Defect Detection & Quality Inspection</p>

            <form onSubmit={handleSubmit} className="bg-black/35 backdrop-blur-md border border-blue-400/20 rounded-2xl p-8 shadow-2xl">
              <label className="block text-xs uppercase tracking-wider text-gray-400 mb-2">Operator Name</label>
              <input
                type="text"
                value={name}
              ...
              <label className="block text-xs uppercase tracking-wider text-gray-400 mb-2">Role</label>
              <div className="grid grid-cols-2 gap-2 mb-6">
                {['Quality Engineer', 'Faculty Inspector'].map((r) => (
                  <button
                    type="button"
                    key={r}
                    onClick={() => setRole(r)}
    ...
    backgroundRepeat: "no-repeat",
        ...
        >
          <div className="text-center relative">
            <div ref={iconBoxRef} className="relative w-36 h-36 mx-auto mb-8">
              <div className="absolute inset-0 flex items-center justify-center">
                <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="text-blue-400/70">
            ...
            <p className="text-white text-xl font-semibold mb-2">Welcome to VisionInspect AI</p>
            <p className="text-blue-400 text-xs tracking-[0.2em] uppercase">Initializing Inspection Console…</p>
          </div>
        </div>
      )}
    </div>
