...
def preprocess_gray(
    image_bgr: np.ndarray, size: tuple[int, int] = (256, 256)
) -> np.ndarray:
...
def build_reference_image(
    train_image_paths: list[str | Path], size: tuple[int, int] = (256, 256)
) -> np.ndarray:
...
def foreground_mask(reference_image: np.ndarray) -> np.ndarray:
    ...
    _, mask = cv2.threshold(
        reference_uint8, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU
    )
...
def build_reference_profile(
    ...
    processed_images = [
        preprocess_gray(load_image_bgr(image_path), size=size)
...
def normalized_anomaly_map(
    ...
    if (
        processed.shape != reference.shape
        or reference.shape != variability.shape
        or mask.shape != reference.shape
    ):
        raise ValueError(
            "Baseline profile dimensions do not match the configured image size."
        )
    if not np.any(mask):
        raise ValueError("Baseline profile foreground mask is empty.")

    brightness_offset = float(np.median((processed - reference)[mask]))
    residual = np.abs(processed - brightness_offset - reference) / (
        variability + variability_floor
    )
...
def anomaly_score(
    diff_map: np.ndarray, percentile: float = 99.0, mask: np.ndarray | None = None
) -> float:
    values = (
        diff_map[np.asarray(mask, dtype=bool)]
        if mask is not None
        else diff_map.reshape(-1)
    )
    if values.size == 0:
        return 0.0
...
def heatmap_overlay(
    image_bgr: np.ndarray, diff_map: np.ndarray, alpha: float = 0.45
) -> np.ndarray:
    image_resized = cv2.resize(
        image_bgr, (diff_map.shape[1], diff_map.shape[0]), interpolation=cv2.INTER_AREA
    )
...
def score_dataframe(
    dataset_df: pd.DataFrame, reference_image: np.ndarray, threshold: float
) -> pd.DataFrame:
    rows = []
